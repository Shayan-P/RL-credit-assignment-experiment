...
class LimitedContextWrapperSortedLength(Dataset):
    """
        this is better than LimitedContextWrapper because it batches the small trajectories together which minimizes
        the amount of padding we need to do

        should not be used with shuffle=True
        and batch_size should be fixed beforehand
    """
    def __init__(self, trajectory_dataset: TrajectoryDataset, context_len, batch_size):

        self.context_len = context_len
        self.trajectory_dataset = [traj for traj in trajectory_dataset]
    ...
    def __getitem__(self, idx):
        ...
        if traj_len >= self.context_len:
            ...
            timesteps = torch.arange(start=0, end=batch_length, step=1)
